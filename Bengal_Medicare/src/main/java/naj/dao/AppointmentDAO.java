...
public class AppointmentDAO {
	...
	public boolean updateCommentStatus(int id, int doctId, String comm) {
	 ...
	 public boolean updateAppointment(Appointment appointment) {
	        ...
	        try {
	            String sql = "UPDATE appointment SET fullname=?, gender=?, email=?, phno=?, diseases=?, status=? WHERE id=? AND doctor_id=?";
	            PreparedStatement ps = conn.prepareStatement(sql);
	            ps.setString(1, appointment.getFullName());
	            ps.setString(2, appointment.getGender());
	            ps.setString(3, appointment.getEmail());
	            ps.setString(4, appointment.getPhNo());
	            ps.setString(5, appointment.getDiseases());
	            ps.setString(6, appointment.getStatus());
	            ps.setInt(7, appointment.getId());
	            ps.setInt(8, appointment.getDoctorId());
	...
	// Add this method to your AppointmentDAO class
	 ...
	 public boolean deleteAppointment(int id, int doctorId) {
	     boolean success = false;
	     try {
	         String sql = "DELETE FROM appointment WHERE id=? AND doctor_id=?";
